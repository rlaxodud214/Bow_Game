...
public class WMG_Series : MonoBehaviour {
	...
	public string formatSeriesDataLabel(WMG_Series series, float val, int labelIndex) {
		...
		if (series.theGraph.useGroups && pointVal.x < 0) { // graph uses groups and series x point is negative always means NULL
			return "";
		}
	...
	void getNewPointPositionsAndSizes(List<GameObject> prevPoints, int index) {
		...
		for (int i = 0; i < pointValuesCount; i++) {
			...
			else if (theGraphUseGroups) { // Using groups, x values represent integer index of group
	...
	void updateAreaShading() {
		...
		if ((areaShadingUsesComputeShader || linesUseComputeShader) && areaShadingRects.Count == 1) { // all values (min, max, and the points) represent a percentage of graph
			int pointValuesCount = pointValues.Count;
			if (pointValuesCount < 2) { // need at least 2 points to draw anything
				theGraph.SetActive(areaShadingRects[0], false);
				return; 
			} else {
			...
			for (int i = 0; i < pointValuesCount; i++) {
				if (neverCreatePoints) { // automatic animations dont work here
